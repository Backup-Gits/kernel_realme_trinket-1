...
void change_charge_current_according_battery_temp(struct oppo_chg_chip *chip);
void lcd_on_change_charge_current_according_skin_temp(struct oppo_chg_chip *chip);
void change_charge_current_according_cpu_temp(struct oppo_chg_chip *chip);

/* Jie1.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Add current limited according to battery tempreture */
static int lcd_on_chg_curr_proc = 2000000;
static int g_batt_temp_status = 0;
/* Jie1.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Add current limited according to skin tempreture */
...
static ssize_t proc_batt_param_noplug_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
		...
		sprintf(read_data, "%d %d %d", 
			noplug_temperature, noplug_batt_volt_max, noplug_batt_volt_min);
...
static ssize_t charging_limit_current_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
        ...
        lcd_on_chg_curr_proc = (limit_current >= 0 ? limit_current : 2000000);
        }
        return len;
}
...
/*vfloat_sw_limit*/
	...
	rc = of_property_read_u32(node, "qcom,ffc2_temp_normal_vfloat_mv", &chip->limits.ffc2_temp_normal_vfloat_mv);
	 ...
	 rc = of_property_read_u32(node, "qcom,ffc1_normal_vfloat_over_sw_limit", &chip->limits.ffc1_normal_vfloat_over_sw_limit);
        if (rc < 0) {
                chip->limits.ffc1_normal_vfloat_over_sw_limit = chip->limits.ffc_normal_vfloat_over_sw_limit;
        }
	 rc = of_property_read_u32(node, "qcom,ffc2_normal_vfloat_over_sw_limit", &chip->limits.ffc2_normal_vfloat_over_sw_limit);
        if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,input_current_vooc_ma_normal", &chip->limits.input_current_vooc_ma_normal);
	 ...
	 rc = of_property_read_u32(node, "qcom,input_current_vooc_ma_led", &chip->limits.input_current_vooc_ma_led);
        if (rc) {
                chip->limits.input_current_vooc_ma_led = 1800;
        }
...
static void oppo_chg_set_input_current_limit(struct oppo_chg_chip *chip)
{
	...
	if ((chip->chg_ctrl_by_calling) && (chip->calling_on) && (current_limit > chip->limits.input_current_calling_ma)) {
	 ...
	 if (chip->chg_ctrl_by_vooc && chip->vbatt_num == 2 
		...
		if(chip->led_on) {
			current_limit = chip->limits.input_current_vooc_ma_led;
		...
		chg_err("chg_ctrl_by_vooc,  led_on = %d,  calling_on = %d, current_limit[%d], chip->vooc_temp_status[%d]\n", chip->led_on, chip->calling_on, current_limit, chip->vooc_temp_status);
...
static void oppo_chg_check_vooc_temp_status(struct oppo_chg_chip *chip)
{
		...
		if (tbat_vooc_status > chip->vooc_temp_status && tbat_vooc_status == VOOC_TEMP_STATUS__WARM)
			chip->limits.vooc_normal_bat_decidegc_antishake = chip->limits.vooc_normal_bat_decidegc - TVOOC_HYSTERISIS_DECIDEGC;
		else if (tbat_vooc_status > chip->vooc_temp_status && tbat_vooc_status == VOOC_TEMP_STATUS__HIGH)
			chip->limits.vooc_warm_bat_decidegc_antishake = chip->limits.vooc_warm_bat_decidegc - TVOOC_HYSTERISIS_DECIDEGC;
		...
		chg_debug("tled status change, [%d %d %d %d]\n", tbat_vooc_status, chip->vooc_temp_status,
			chip->limits.vooc_warm_bat_decidegc_antishake, chip->limits.vooc_normal_bat_decidegc_antishake);
...
/* Jie1.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Add current limited according to battery tempreture */
#define TEMP_BATT_370 (370)
#define TEMP_BATT_350 (350)

/* Jie1.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Add current limited according to skin tempreture */
#define TEMP_SKIN_390 (390)
#define TEMP_SKIN_370 (370)
#define TEMP_SKIN_350 (350)
#define TEMP_SKIN_LEVEL_0 (-400)

/* Jie1.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Add current limited according to cpu tempreture */
#define TEMP_CPU_460 (460)
#define TEMP_CPU_430 (430)
...
void change_charge_current_according_battery_temp(struct oppo_chg_chip *chip)
{
    ...
    static int batt_temp_38_flag = 0;
    static int batt_temp_36_flag = 0;
    static int batt_temp_34_flag = 0;
    static int batt_temp_change_flag = 0;

    if (g_batt_temp_status == 0) {
        if (chip->temperature > TEMP_BATT_370) {
            g_batt_temp_status = 38;
        } else if (chip->temperature > TEMP_BATT_350) {
            g_batt_temp_status = 36;
        } else {
            g_batt_temp_status = 34;
        }
        batt_temp_38_flag = 0;
        batt_temp_36_flag = 0;
        batt_temp_34_flag = 0;
    }
    if (g_batt_temp_status == 34) {
        if (chip->temperature > TEMP_BATT_370) {
            batt_temp_38_flag++;
            batt_temp_36_flag++;
            batt_temp_34_flag = 0;
            if (batt_temp_38_flag > 2) {
                g_batt_temp_status = 38;
                batt_temp_change_flag = 1;
            } else if (batt_temp_36_flag > 2) {
                g_batt_temp_status = 36;
                batt_temp_change_flag = 1;
            }
        } else if (chip->temperature > TEMP_BATT_350) {
        ...
        if (chip->temperature > TEMP_BATT_370) {
        ...
        } else if (chip->temperature > TEMP_BATT_350 - 10) {
            batt_temp_38_flag = 0;
            batt_temp_36_flag = 0;
            batt_temp_34_flag = 0;
        } else {
            ...
            batt_temp_36_flag = 0;
            batt_temp_38_flag = 0;
            if (batt_temp_34_flag > 2) {
                g_batt_temp_status = 34;
                batt_temp_change_flag = 1;
            }
        }
    } else if (g_batt_temp_status == 38) {
        if (chip->temperature > TEMP_BATT_370 - 10) {
            batt_temp_38_flag = 0;
            batt_temp_36_flag = 0;
            batt_temp_34_flag = 0;
        } else if (chip->temperature > TEMP_BATT_350 - 10) {
...
void lcd_on_change_charge_current_according_skin_temp(struct oppo_chg_chip *chip)
{
    ...
    if (g_skin_temp_status_1 == 0) {
        if (chip->odm_skin_temp > TEMP_SKIN_390) {
            g_skin_temp_status_1 = 40;
            vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_1200_MA);
        } else if (chip->odm_skin_temp > TEMP_SKIN_370) {
            g_skin_temp_status_1 = 38;
            vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_1500_MA);
        } else {
            g_skin_temp_status_1 = 36;
            vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_2000_MA);
        }
    }
    if (g_skin_temp_status_1 == 36) {
        if (chip->odm_skin_temp > TEMP_SKIN_390) {
            g_skin_temp_status_1 = 40;
            vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_1200_MA);
        } else if (chip->odm_skin_temp > TEMP_SKIN_370) {
    ...
    } else if (g_skin_temp_status_1 == 38) {
        if (chip->odm_skin_temp > TEMP_SKIN_390) {
            g_skin_temp_status_1 = 40;
            vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_1200_MA);
        } else if (chip->odm_skin_temp <= TEMP_SKIN_370 - 10) {
            g_skin_temp_status_1 = 36;
            vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_2000_MA);
        } else {
            vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_1500_MA);
        ...
        if (chip->odm_skin_temp <= TEMP_SKIN_370 - 10) {
            g_skin_temp_status_1 = 36;
            vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_2000_MA);
        } else if (chip->odm_skin_temp <= TEMP_SKIN_390 - 10) {
            g_skin_temp_status_1 = 38;
            vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_1500_MA);
        } else {
            vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_1200_MA);
...
void change_charge_current_according_cpu_temp(struct oppo_chg_chip *chip)
{
    ...
    if (g_cpu_temp_status_1 == 0) {
        if (chip->odm_cpu_temp > TEMP_CPU_460) {
            g_cpu_temp_status_1 = 47;
        } else if (chip->odm_cpu_temp > TEMP_CPU_430) {
            g_cpu_temp_status_1 = 45;
        } else {
            g_cpu_temp_status_1 = 42;
        }
    }
    if (g_cpu_temp_status_1 == 42) {
        if (chip->odm_cpu_temp > TEMP_CPU_460) {
            g_cpu_temp_status_1 = 47;
        } else if (chip->odm_cpu_temp > TEMP_CPU_430) {
            g_cpu_temp_status_1 = 45;
        }
    } else if (g_cpu_temp_status_1 == 45) {
        if (chip->odm_cpu_temp > TEMP_CPU_460) {
            g_cpu_temp_status_1 = 47;
        } else if (chip->odm_cpu_temp <= TEMP_CPU_430 - 10) {
            g_cpu_temp_status_1 = 42;
        }
    } else {
        if (chip->odm_cpu_temp <= TEMP_CPU_430 - 10) {
            g_cpu_temp_status_1 = 42;
        } else if (chip->odm_cpu_temp <= TEMP_CPU_460 - 10) {
            g_cpu_temp_status_1 = 45;
        }
    }
    if (g_cpu_temp_status_1 == 42)
        ...
        vote(chg->usb_icl_votable, CPU_TEMP_VOTER, true, TEMP_CURRENT_1500_MA);
...
static void oppo_chg_batt_skin_cpu_temp_set_current(struct oppo_chg_chip *chip)
{
    ...
    if (chg->iio.odm_skin_temp_chan) {
        rc = iio_read_channel_processed(chg->iio.odm_skin_temp_chan, &chip->odm_skin_temp);
        if (rc < 0) {
            pr_debug("Couldn't read SKIN TEMP channel, rc=%d\n", rc);
            chip->odm_skin_temp = 25000;
        }
        chip->odm_skin_temp /= 100;
    }

    if (chg->iio.odm_cpu_temp_chan) {
        rc = iio_read_channel_processed(chg->iio.odm_cpu_temp_chan, &chip->odm_cpu_temp);
        if (rc < 0) {
            pr_debug("Couldn't read CPU0 TEMP channel, rc=%d\n", rc);
            chip->odm_cpu_temp = 25000;
        }
        chip->odm_cpu_temp /= 100;
    }

    /* Bin2.Zhang@ODM_WT.BSP.Charger.Basic.1941873, 20200405, Add for monitor skin/cpu temperature */
    if ((chip->chg_ctrl_by_lcd) && (chip->led_on)) {
        /* Jie1.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Add current limited according to battery tempreture */
        change_charge_current_according_battery_temp(chip);
        /* Jie1.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Add current limited according to skin tempreture */
        lcd_on_change_charge_current_according_skin_temp(chip);
        /* Jie1.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Add current limited according to cpu tempreture */
        change_charge_current_according_cpu_temp(chip);
    } else {
		...
		if (((oppo_vooc_get_fastchg_to_normal()== true) || (oppo_vooc_get_fastchg_to_warm() == true))
			...
			if (fastchg_present_wait_count == FULL_DELAY_COUNTS && chip->chg_ops->get_charging_enable() == false
				...
				if (chip->ffc_support && chip->ffc_temp_status != FFC_TEMP_STATUS__HIGH
					...
					if (chip->vbatt_num == 2 && chip->chg_ctrl_by_vooc == false) {
						oppo_chg_turn_on_ffc2(chip);
					} else {
...
static void oppo_chg_print_log(struct oppo_chg_chip *chip)
{
	...
	charger_xlog_printk(CHG_LOG_CRTI, " CHGR[ %d / %d / %d / %d / %d ], BAT[ %d / %d / %d / %d / %d / %d ], GAUGE[ %d / %d / %d / %d / %d / %d / %d / %d / %d ], "
		"STATUS[ 0x%x / %d / %d / %d / %d / 0x%x ], OTHER[ %d / %d / %d / %d / %d/ %d ], ODM[ C%d / S%d ]\n",
        ...
        chip->odm_cpu_temp, chip->odm_skin_temp);
...
static void oppo_chg_pd_config(struct oppo_chg_chip *chip)
{
	...
	if (pd_chging == false && chip->chg_ops->oppo_chg_get_pd_type() == true) {
		pd_chging = true;
...
bool oppo_chg_get_cool_down_status(void)
{
      if (!g_charger_chip) {
                return 0;
        } else {
